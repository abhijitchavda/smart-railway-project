...
header('Content-type: application/json');
  echo json_encode(array('result'=>false,'code'=>201,'type'=>'ON_PLATFORM','msg'=>'no active saved tickets'));
}
else
{
...
foreach ($res1['trains'] as $user) {
    if($user['train']['number']==$train_no)
    {
        ...
        $cancelled=1;
        header('Content-type: application/json');
  echo json_encode(array('result'=>false,'type'=>'ON_PLATFORM','code'=>501,'msg'=>'your train has been cancelled'));
  break;
    }
}
}
...
{
  //------------------------------------------------------------
  $query1="select compartment from resticket where email='$email';";
$result1=@mysql_query($query1);
$rows1=@mysql_fetch_array($result1);
$compartment=$rows1['compartment'];

 
  $query2="select comp_id,direction from comp_pos_12934 where comp_type='$compartment';";
...
if(!$rows2)
{
   if($type=="RESERVATION")
    {
    header('Content-type: application/json');
  echo json_encode(array('result'=>true,'code'=>200,'type'=>'ON_PLATFORM','comp_location'=>'N','ticket'=>'RESERVATION','current_position'=>$status,'late'=>$late,'platform_no'=>$platform_no,'sch_arr_time'=>$schtime,'act_arr_time'=>$acctime,'train_no'=>$train_no,'train_name'=>$train_name,'msg'=>'you have entered invalid compartment or have not entered one.'));
}
if($type=="GENERAL")
{
...
$rows=@mysql_num_rows($result1);
    header('Content-type: application/json');
  echo json_encode(array('result'=>true,'code'=>200,'type'=>'ON_PLATFORM','comp_location'=>'N','gen_people'=>$rows,'ticket'=>'GENERAL','current_position'=>$status,'late'=>$late,'platform_no'=>$platform_no,'sch_arr_time'=>$schtime,'act_arr_time'=>$acctime,'train_no'=>$train_no,'train_name'=>$train_name,'msg'=>'you have entered invalid compartment or have not entered one.'));
...
$comp_position=$rows2['comp_id'];
$comp_direction=$rows2['direction'];
 //------------------------------------------------------
    if($type=="RESERVATION")
    {
    header('Content-type: application/json');
  echo json_encode(array('result'=>true,'code'=>200,'type'=>'ON_PLATFORM','comp_location'=>'Y','ticket'=>'RESERVATION','current_position'=>$status,'late'=>$late,'platform_no'=>$platform_no,'sch_arr_time'=>$schtime,'act_arr_time'=>$acctime,'train_no'=>$train_no,'train_name'=>$train_name,'comp_position'=>$comp_position,'comp_direction'=>$comp_direction));
}
if($type=="GENERAL")
{
$query1="SELECT * FROM gen";
...
$rows=@mysql_num_rows($result1);
    header('Content-type: application/json');
  echo json_encode(array('result'=>true,'code'=>200,'type'=>'ON_PLATFORM','comp_location'=>'Y','gen_people'=>$rows,'ticket'=>'GENERAL','current_position'=>$status,'late'=>$late,'platform_no'=>$platform_no,'sch_arr_time'=>$schtime,'act_arr_time'=>$acctime,'train_no'=>$train_no,'train_name'=>$train_name,'comp_position'=>$comp_position,'comp_direction'=>$comp_direction));
...
{
header('Content-type: application/json');
  echo json_encode(array('result'=>false,'code'=>201,'type'=>'ON_PLATFORM','msg'=>'Train does not stop on this station','train_route'=>$station_list));
...
{
header('Content-type: application/json');
  echo json_encode(array('result'=>false,'code'=>201,'type'=>'ON_PLATFORM','msg'=>'no response data from API'));
...
{
header('Content-type: application/json');
  echo json_encode(array('result'=>false,'code'=>201,'type'=>'ON_PLATFORM','msg'=>'hits finish'));
...
{
header('Content-type: application/json');
  echo json_encode(array('result'=>false,'code'=>201,'type'=>'ON_PLATFORM','msg'=>'Train not running on this date'));
...
{
    header('Content-type: application/json');
  echo json_encode(array('result'=>false,'code'=>202,'type'=>'ON_PLATFORM','msg'=>'error:server failure'));

...
header('Content-type: application/json');
  echo json_encode(array('result'=>false,'code'=>201,'type'=>'ON_ENTERENCE','msg'=>'no active saved tickets'));
...
foreach ($res1['trains'] as $user) {
	if($user['train']['number']==$train_no)
	{
		...
		$cancelled=1;
		header('Content-type: application/json');
  echo json_encode(array('result'=>false,'type'=>'ON_ENTERENCE','code'=>501,'msg'=>'your train has been cancelled'));
  break;
	}
}
}
...
if($count==1)
{
	...
	{
	header('Content-type: application/json');
  echo json_encode(array('result'=>true,'code'=>200,'type'=>'ON_ENTERENCE','ticket'=>'RESERVATION','current_position'=>$status,'late'=>$late,'platform_no'=>$platform_no,'sch_arr_time'=>$schtime,'act_arr_time'=>$acctime,'train_route'=>$station_list,'train_no'=>$train_no,'train_name'=>$train_name));
}
if($type=="GENERAL")
{
...
$rows=@mysql_num_rows($result1);
	header('Content-type: application/json');
  echo json_encode(array('result'=>true,'code'=>200,'type'=>'ON_ENTERENCE','gen_people'=>$rows,'ticket'=>'GENERAL','current_position'=>$status,'late'=>$late,'platform_no'=>$platform_no,'sch_arr_time'=>$schtime,'act_arr_time'=>$acctime,'train_route'=>$station_list,'train_no'=>$train_no,'train_name'=>$train_name));
...
{
header('Content-type: application/json');
  echo json_encode(array('result'=>false,'code'=>201,'type'=>'ON_ENTERENCE','msg'=>'Train does not stop on this station','train_route'=>$station_list));
...
{
header('Content-type: application/json');
  echo json_encode(array('result'=>false,'code'=>201,'type'=>'ON_ENTERENCE','msg'=>'no response data from API'));
...
{
header('Content-type: application/json');
  echo json_encode(array('result'=>false,'code'=>201,'type'=>'ON_ENTERENCE','msg'=>'hits finish'));
...
{
header('Content-type: application/json');
  echo json_encode(array('result'=>false,'code'=>201,'type'=>'ON_ENTERENCE','msg'=>'Train not running on this date'));
...
{
	header('Content-type: application/json');
  echo json_encode(array('result'=>false,'code'=>202,'type'=>'ON_ENTERENCE','msg'=>'error:server failure'));
